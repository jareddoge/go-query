            ...
            <div className="note-input-area">
                {tasks.map((task, index) => (
                    <div key={task.id} className="task-item" style={{ justifyContent: task.text !== '' ? 'space-between' : 'none', paddingTop: index === 0 ? 8 : 0 }}>
                        ...
                        <input type="text" onSelect={(e) => onShortcut(e, 'select')} onKeyDown={(e) => onShortcut(e, 'keyDown')} onKeyUp={(e) => onShortcut(e, 'keyUp')} style={task.checked ? { textDecoration: 'line-through' } : {}} id={`${id}taskInput${task.id}`}
                            onChange={(e) => onChangeText(e, index)}></input>
                        {task.text !== '' &&
